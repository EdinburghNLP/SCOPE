#!/usr/bin/env bash
# train_qwen3_8b.sh — SCOPE long-form training pipeline
# (6-GPU H100 layout, Qwen3-8B with thinking-hidden per-turn datums).
#
# Base: training/train_qwen25_7b.sh (config/structure) with Qwen3 adaptations:
#   1. Qwen3-8B model with native hermes/tool_call format.
#   2. Thinking-hidden mode: custom chat template (qwen3_strip_thinking.jinja)
#      that strips <think> blocks from previous assistant turns in tool-use
#      conversations.
#   3. Per-turn datum splitting handles the extension property break — each
#      training datum has thinking stripped from previous turns (matching
#      inference) and preserved for the current turn.
#   4. TP=1 for rollout (Qwen3-8B fits single H100 80GB).
#   5. BATCH_PER_GPU=1 (reduced for Qwen3-8B memory with thinking tokens).
#   6. QUALITY_MAX_TOKENS=1024 (Qwen3 thinking needs >256 tokens for <score>).
#
# 6-GPU layout:
#   GPUs 0-1 (SERVER_GPUS): Retrieval (FAISS) + inference servers
#   GPUs 2-5 (TRAIN_GPUS):  Training + rollout (TP=1, DP=4)
#
# Challenger iter1: solver on SERVER_GPUS (DP=2).
# Challenger iter2+: solver on SOLVER_GPU (GPU 0), grader on GRADER_GPU (GPU 1).
# Solver: grader on SERVER_GPUS (DP=2).
#
# Recovery: use START_ITER and START_STAGE to resume from a specific point.
#   START_ITER=2 START_STAGE=3 bash training/train_qwen3_8b.sh
#
# Usage:
#   bash training/train_qwen3_8b.sh
#   NUM_ITERATIONS=5 bash training/train_qwen3_8b.sh
#   START_ITER=2 START_STAGE=3 bash training/train_qwen3_8b.sh
...
SOLVER_GPU=${SOLVER_GPU:-"0"}            # Solver server (iter2+ challenger: dedicated GPU)
GRADER_GPU=${GRADER_GPU:-"1"}            # Grader server (iter2+ challenger: dedicated GPU)
...
TP=1                                     # Tensor parallel (Qwen3-8B fits single H100)
DP=2                                     # Data parallel (challenger servers)
SERVER_TYPE=sglang

# --- Tool format (Qwen3 native hermes/tool_call) ---
TOOL_FORMAT=${TOOL_FORMAT:-"native"}
TOOL_CALL_PARSER_TYPE="qwen25"

if [ "$TOOL_FORMAT" = "native" ]; then
    ROLLOUT_FORMAT="hermes"
    CHALLENGER_FMT="tool_call"
    SOLVER_FMT="tool_call"
    CHALLENGER_STOP='["</tool_call>","</task>"]'
    SOLVER_STOP='["</answer>"]'
    TASK_FORMAT="tool_call"
    CHALLENGER_TEMPLATE="./scope/prompts/challenger_tool_call.txt"
    SOLVER_TEMPLATE="./scope/prompts/solver_tool_call.txt"
else
    ROLLOUT_FORMAT="search_r1"
    CHALLENGER_FMT="search_r1"
    SOLVER_FMT="search_r1"
    CHALLENGER_STOP='["</search>","</task>"]'
    SOLVER_STOP='["</answer>"]'
    TASK_FORMAT="search_r1"
    CHALLENGER_TEMPLATE="./scope/prompts/challenger_search_r1.txt"
    SOLVER_TEMPLATE="./scope/prompts/solver_search_r1.txt"
fi
...
GRPO_GROUP_SIZE=8
REWARD_ROLLOUT_N=8
BATCH_PER_GPU=1                          # reduced from 2: Qwen3 thinking tokens increase memory
SOLVER_BATCH_SIZE=256
TRAINER_MAX_EPOCHS=1000000

# --- Templates (CHALLENGER_TEMPLATE and SOLVER_TEMPLATE set by TOOL_FORMAT above) ---
CHAT_TEMPLATE_PATH="./scope/chat_templates/qwen3_strip_thinking.jinja"
GRADER_TEMPLATE_CHALLENGER="./scope/prompts/grader_per_rubric_qwen3.txt"
GRADER_TEMPLATE_SOLVER="./scope/prompts/grader_per_rubric_qwen3.txt"
RUBRIC_TEMPLATE="./scope/prompts/rubric.txt"
TASK_DESCRIPTIONS_DIR="./scope/prompts/tasks"

# --- Dynamic user turns ---
DYNAMIC_USER_TURNS=1
DYNAMIC_STYLE="chain2"
...
QUALITY_MAX_TOKENS=1024  # Qwen3 thinking needs >256 tokens for <score> tag

# --- Data ---
CHALLENGER_BASE_SEED=42
CHALLENGER_VAL_DATA="./data/validation_qwen3.parquet"
SOLVER_VAL_DATA="./data/validation_qwen3.parquet"
...
CHALLENGER_ROLLOUT_MEM_ITER1=0.35
CHALLENGER_SOLVER_MEM_ITER1=0.50
CHALLENGER_ROLLOUT_MEM=0.35
CHALLENGER_SOLVER_MEM=0.60
CHALLENGER_GRADER_MEM=0.60

SOLVER_ROLLOUT_MEM=0.80
...
echo ""
echo "=================================================================="
echo "  SCOPE Training Pipeline (Qwen3 thinking-hidden, 6-GPU H100)"
...
echo "  Tool format: ${TOOL_FORMAT} (rollout=${ROLLOUT_FORMAT})"
echo "  Server GPUs: ${SERVER_GPUS} | Train GPUs: ${TRAIN_GPUS}"
echo "  TP=${TP}, DP=${DP}, Train N_GPUs=${TRAIN_N_GPUS}"
echo "  Per-turn datums: enabled (thinking-hidden)"
echo "=================================================================="
echo ""
...
run_challenger() {
    ...
    echo "  Servers: GPUs ${SERVER_GPUS} | Training: GPUs ${TRAIN_GPUS}"
    echo "################################################################"
    echo ""
    ...
    if [ $iter -eq 1 ]; then
        # Iter 1: solver = grader = base model, both on port 8001 (SERVER_GPUS, DP=2)
        ...
        # Iter 2+: trained solver on 8001 (SOLVER_GPU), base grader on 8002 (GRADER_GPU)
    ...
    if [ $iter -eq 1 ]; then
        # Iter 1: solver on both server GPUs (DP=2)
        start_server "$SERVER_GPUS" 8001 "$solver_model" $DP $TP $solver_mem $SERVER_TYPE \
            "logs/pipeline_solver_challenger_iter${iter}.log" "$solver_served_name"
        SERVER_8001_PID=$!
    else
        # Iter 2+: solver on dedicated GPU (DP=1), grader on dedicated GPU (DP=1)
        start_server "$SOLVER_GPU" 8001 "$solver_model" 1 $TP $solver_mem $SERVER_TYPE \
            "logs/pipeline_solver_challenger_iter${iter}.log" "$solver_served_name"
        SERVER_8001_PID=$!
    fi
    ...
    # Grader server (port 8002, iter2+ only -- always base model, on dedicated GPU)
    if [ $iter -gt 1 ]; then
        start_server "$GRADER_GPU" 8002 "$BASE_MODEL" 1 $TP $CHALLENGER_GRADER_MEM $SERVER_TYPE \
    ...
    CUDA_VISIBLE_DEVICES=$TRAIN_GPUS python -m verl.trainer.main_ppo \
        ...
        +actor_rollout_ref.rollout.multi_turn.tool_call_parser_type=${TOOL_CALL_PARSER_TYPE} \
        actor_rollout_ref.rollout.multi_turn.use_inference_chat_template=True \
        actor_rollout_ref.rollout.multi_turn.tokenization_sanity_check_mode=disable \
        actor_rollout_ref.rollout.multi_turn.per_turn_datums=True \
...
run_create_tasks() {
    local iter=$1
    local final_parquet_early="./data/gen_filtered_iter${iter}.parquet"
    ...
    local final_parquet="./data/gen_filtered_iter${iter}.parquet"
    local run_dir="./data/pipeline_iter${iter}"
...
run_solver() {
    ...
    local train_data="./data/gen_filtered_iter${iter}.parquet"
