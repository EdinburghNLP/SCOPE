#!/usr/bin/env bash
# train_qwen25_7b.sh — SCOPE long-form training pipeline
# (Qwen2.5-7B-Instruct, 6-GPU H100 layout).
#
# 6-GPU layout:
#   GPUs 0-1 (SERVER_GPUS): Retrieval (FAISS) + inference servers (grader, solver rollout for task creation)
#   GPUs 2-5 (TRAIN_GPUS):  Training + rollout (TP=1, DP=4, param_offload + multi_stage_wake_up)
#
...
#   START_ITER=2 START_STAGE=3 bash training/train_qwen25_7b.sh
#
# Usage:
#   bash training/train_qwen25_7b.sh
#   NUM_ITERATIONS=5 bash training/train_qwen25_7b.sh
#   START_ITER=2 START_STAGE=3 bash training/train_qwen25_7b.sh
...
# --- Model & hardware (6-GPU H100 layout, overridable) ---
...
TP=2                                     # Tensor parallel (challenger servers)
...
# --- Quality grading ---
# Comma-separated gate names: entity, no_leakage, source_relevance (any subset).
# Empty string disables quality gating entirely.
...
# Minimum number of quality gates that must pass (default: all).
QUALITY_REQUIRED_SUM=2

# --- Data ---
CHALLENGER_BASE_SEED=42
CHALLENGER_VAL_DATA="./data/validation_qwen25.parquet"
SOLVER_VAL_DATA="./data/validation_qwen25.parquet"
...
# Challenger: servers share SERVER_GPUS (0,1), training on TRAIN_GPUS (2-5)
...
CHALLENGER_GRADER_MEM=0.65        # iter2+ (SGLang formula: high fraction = less reserved non-cache; FAISS eats ~53GB/GPU)
...
echo ""
echo "=================================================================="
echo "  SCOPE Training Pipeline (batch${SOLVER_BATCH_SIZE} solver, 6-GPU H100 layout)"
...
run_challenger() {
    ...
    local iter_train_data="./data/challenger_iter${iter}_seed${iter_seed}.parquet"
    ...
    CUDA_VISIBLE_DEVICES=$TRAIN_GPUS python -m verl.trainer.main_ppo \
        ...
        '+actor_rollout_ref.rollout.stop=["</search>","</task>"]' \
        ...
        '+custom_reward_function.reward_kwargs.solver_stop_tokens=["</answer>"]' \
        +custom_reward_function.reward_kwargs.challenger_format="search_r1" \
        +custom_reward_function.reward_kwargs.solver_format="search_r1" \
