...
def _release_cpu_memory():
    ...
    glibc's malloc retains freed arena pages by default, causing RSS to
    ratchet upward after large transient allocations (e.g. checkpoint
    state-dict materialisation).  ``malloc_trim(0)`` asks glibc to
    release as many pages as possible back to the OS.
    """
    gc.collect()
    try:
        ctypes.CDLL("libc.so.6").malloc_trim(0)
    except Exception:
        pass  # non-glibc platform (macOS, musl) — skip silently
...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None,
        ...
        # Reclaim accumulated freed-but-retained heap pages *before* the
        # save so the baseline RSS is as low as possible.
        _release_cpu_memory()
        ...
        # Save model and optimizer in *separate* FSDP contexts so that
        # FSDP internal references to flat-param shards are released
        # (and pages returned to the OS) before the optimizer allocation.
        ...
        # Permanent checkpoints never enter the deletable queue, so the
        # rotation logic in future saves will never remove them.
        if not is_permanent:
            self.previous_saved_paths.append(local_path)

        # Rotate AFTER the new checkpoint is fully on disk, so a crash during
        # save leaves the previous good actor intact. Rank 0 only.
        if (
            self.rank == 0
            ...
            self.remove_previous_save_local_path(self.previous_saved_paths[:remove_count])
