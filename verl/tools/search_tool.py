...
class SearchTool(BaseTool):
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[str, float, dict]:
        ...
        # Normalize query items: Qwen3 sometimes generates {"query": "..."} dicts
        # instead of plain strings. Extract the string value from dicts.
        normalized = []
        for item in query_list_from_params:
            if isinstance(item, str):
                normalized.append(item)
            elif isinstance(item, dict):
                for key in ("query", "text", "content", "q"):
                    if key in item and isinstance(item[key], str):
                        normalized.append(item[key])
                        break
                else:
                    # Fallback: take the first string value
                    for v in item.values():
                        if isinstance(v, str):
                            normalized.append(v)
                            break
        if not normalized:
            error_msg = "Error: 'query_list' contained no extractable string queries."
            logger.error(f"[SearchTool] {error_msg} Received parameters: {parameters}")
            return json.dumps({"result": error_msg}), 0.0, {}
        query_list_from_params = normalized
