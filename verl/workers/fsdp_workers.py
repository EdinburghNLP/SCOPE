...
from verl.utils import hf_processor, hf_tokenizer, load_chat_template
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        if not torch.distributed.is_initialized():
            ...
            torch.distributed.init_process_group(
                ...
                timeout=timedelta(seconds=int(os.environ.get("VERL_DISTRIBUTED_TIMEOUT", 7200))),
            )
    ...
    def _build_model_optimizer(
        ...
        if self.config.model.get("custom_chat_template", None) is not None:
            template = load_chat_template(self.config.model.custom_chat_template)
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None, is_permanent=False):
        ...
        self.checkpoint_manager.save_checkpoint(
            ...
            max_ckpt_to_keep=max_ckpt_to_keep, is_permanent=is_permanent
        )
        dist.barrier()
