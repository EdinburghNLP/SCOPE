...
class AsyncRolloutRequest(BaseModel):
    ...
    per_turn_prompts: list[str] = []  # Decoded per-turn prompts (validation only)

    _left_truncated_count: int = PrivateAttr(default=0)
    ...
    base_str_with_gen_len: int = 0
    base_str_wo_gen_len: int = 0
    ...
    def initialize_request(cls, values):
        ...
        values["base_str_with_gen_len"] = len(
        ...
        values["base_str_wo_gen_len"] = len(
            cls._handle_apply_chat_template(
                processing_class,
                BASE_CHAT_HISTORY,
                multi_modal_data=multi_modal_data,
    ...
    def _tokenize_content(
        ...
        """Tokenize a content string without adding BOS/EOS special tokens.

        This is used to re-tokenize content extracted via string-level slicing,
        avoiding BPE token boundary merge issues that occur with token-level slicing.

        Args:
            processing_class: The tokenizer or processor to use for tokenization.
            text: The content string to tokenize.

        Returns:
            A 2D tensor of token IDs with shape (1, seq_len).

        Raises:
            ValueError: If the processing class type is not supported.
        """
        if isinstance(processing_class, (PreTrainedTokenizer, PreTrainedTokenizerFast)):
            return processing_class(text=[text], add_special_tokens=False, return_tensors="pt")["input_ids"]
        elif isinstance(processing_class, ProcessorMixin):
            return processing_class.tokenizer(text=[text], add_special_tokens=False, return_tensors="pt")["input_ids"]
        else:
            raise ValueError(f"Unsupported processing class type: {type(processing_class)}")
    ...
    def _left_truncate_rolling_context(self, processing_class, keep_length: int) -> int:
        """Left-truncate input_ids and associated tensors to keep_length.

        Drops the oldest tokens (from the left) so that the rolling context
        fits within ``keep_length`` tokens, matching Search-R1's behavior
        of truncating old context instead of stopping the rollout.

        Args:
            processing_class: Tokenizer/processor (unused but kept for API
                consistency with other methods).
            keep_length: Target sequence length to keep.

        Returns:
            int: Number of tokens dropped from the left.
        """
        current_len = self.input_ids.shape[-1]
        if current_len <= keep_length:
            return 0
        if self.multi_modal_inputs:
            logger.warning(
                "Rolling context truncation with multimodal inputs is unsafe "
                "(image/video tensors are not synchronized with token indices). "
                "Skipping truncation."
            )
            return 0
        drop = current_len - keep_length
        self.input_ids = self.input_ids[..., drop:]
        self.attention_mask = self.attention_mask[..., drop:]
        self.loss_mask = self.loss_mask[..., drop:]
        self.position_ids = compute_position_id_with_mask(self.attention_mask)
        self._left_truncated_count += drop
        return drop
    ...
    def add_user_message(
        ...
        messages = [*BASE_CHAT_HISTORY, self.messages[-1].model_dump(exclude_none=True)]
        tools = [tool.model_dump() for tool in self.tool_schemas] if self.tool_schemas else None

        # String-level slicing to avoid BPE token boundary merge issues.
        # Token-level slicing can lose leading whitespace when BPE merges characters across
        # the base/content boundary into a single token.
        full_str = self._handle_apply_chat_template(
            processing_class, messages, multi_modal_data={}, tools=tools, add_generation_prompt=False, tokenize=False
        )
        content_str = full_str[self.base_str_wo_gen_len :]
        content_ids = self._tokenize_content(processing_class, content_str)
    ...
    def add_assistant_message(
        ...
        content_str = full_str[self.base_str_with_gen_len :]
        content_ids = self._tokenize_content(processing_class, content_str)
    ...
    def add_tool_response_messages(
        ...
        messages = [*BASE_CHAT_HISTORY, *[m.model_dump(exclude_none=True) for m in self.messages[-len(contents) :]]]
        tools = [tool.model_dump() for tool in self.tool_schemas] if self.tool_schemas else None
        ...
        has_multi_modal = any(len(v) > 0 for v in delta_multi_modal_data.values())

        if has_multi_modal:
            # For multi-modal content, fall back to token-level slicing (unchanged behavior)
            # because _tokenize_content only handles text.
    ...
    def finalize(
        ...
        if self._left_truncated_count > 0:
            # Skip sanity check — input_ids has been left-truncated for rolling context,
            # so re-tokenizing the full message list will not match the truncated input_ids.
            pass
        elif self.tokenization_sanity_check_mode != TokenizationSanityCheckModeEnum.DISABLE:
    ...
    def truncate_output_ids(
        ...
        # When left-truncation has occurred, input_ids is shorter than
        # prompt_ids (which records the *original* prompt).  Compute the
        # effective prompt length that still remains in input_ids.
        effective_prompt_len = max(0, self.prompt_ids.shape[-1] - self._left_truncated_count)
        self.response_ids = self.input_ids[..., effective_prompt_len:][..., : self.max_response_len]
        effective_prompt_attn_len = max(0, self.prompt_attention_mask.shape[-1] - self._left_truncated_count)
        self.response_attention_mask = self.attention_mask[..., effective_prompt_attn_len:][
            ..., : self.max_response_len
        ]
        effective_prompt_pos_len = max(0, self.prompt_position_ids.shape[-1] - self._left_truncated_count)
        self.response_position_ids = self.position_ids[..., effective_prompt_pos_len:][
            ..., : self.max_response_len
        ]
        effective_prompt_loss_len = max(0, self.prompt_loss_mask.shape[-1] - self._left_truncated_count)
        self.response_loss_mask = self.loss_mask[..., effective_prompt_loss_len:][..., : self.max_response_len]
