...
class FSDPSGLangShardingManager(BaseShardingManager):
    ...
    def __init__(
        ...
        # NOTE: _skip_next_sync is disabled — FSDP init corrupts the weights
        # that SGLang loaded during multi_stage_wake_up, so the first sync
        # is actually required.  With dp>1 the per-GPU FSDP shard is small
        # enough that state_dict() fits alongside SGLang.
    ...
    def __enter__(self):
        ...
        if self._skip_next_sync:
            self._skip_next_sync = False
            logger.info("Skipping first weight sync (multi_stage_wake_up: weights already in SGLang)")
            return
